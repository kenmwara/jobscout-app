...
| `node tools/check_field_chrome.mjs` | No control shows its UA defaults: every textarea computes resize none; a control inside `.field` draws no ring of its own (the well lights on focus-within) while one outside `.field` still does; an empty control is not scrolled or clipped by its own placeholder; every résumé control lives inside a `.field` that offers both doors (paste area + file input). Site landing/apply/browse and the mockup home/matches/draft, light and dark. |
| `node tools/check_bar_bleed.mjs` | A bar that paints, bleeds: any header/toolbar that paints a background must reach both edges of its scroll container, else the ground shows beside it as a seam (the mockup's sticky header, painted canvas inside the view's padding, drew two lines down the market wash). Geometry, so it catches the next one. |
