...
**Stage 1 is written.** `stage1/card.css`, `stage1/card.html`, `stage1/card.js`.
The two card assertions above are now `tools/check_card.mjs`, with three
mutations (`sans-title`, `chip-equals-band`, `flat-date`). All four checks run
green against the written files; every mutation breaks them.

**Two seams found while writing it, both of which outlive Stage 1:**

1. **The band tokens have two spellings and both ship.**
   `theme-resolution.css` says `--auto` / `--auto-label` / `--auto-fill` /
   `--nearmiss`; `site/base.css` says `--b-auto` / `--b-auto-l` / `--b-auto-bg`
   / `--b-near`. A rule naming one renders unstyled against the other, and
   *silently* — an undefined custom property is not an error. `card.css` reads
   every band once through a `var(--b-auto, var(--auto))` fallback chain and
   re-exports it locally, so the card binds to whichever exists. That bridge is
   a bandage; one spelling should win. Neither file declares a mono stack at
   all, though law 13 assigns JetBrains Mono to data.

2. **`--sunken` is invisible on dark.** Measured against `--surface`:
   light `#ece5d8` is **1.253:1**, dark `#1a1240` is **1.030:1**. §10 already
   says dark separates by *lifting* — there is no shadow to sit in, so a sunken
   container on ink has nothing to read against. This is not a chip problem;
   it is every sunken container in the product. `card.css` picks
   `light-dark(var(--sunken), var(--evidence-bg))` for tier 2, which restores
   the 1.225:1 step, but the fix belongs in the token file.
