 ...
 *
 *   A  the text column keeps most of the field's width
 *   B  the radius steps down from the pill to the card
 *   C  the controls sit on ONE row beneath the text, clip first, go last
 *   D  the EMPTY bar is untouched: one line, still a pill   (the guard on A-C)
 *
 *   node tools/check_field_phone.mjs
 *   node tools/check_field_phone.mjs --mutate oval | crushed | clip-adrift | empty-grew
 */
...
for (const width of [360, 390, 430]) {
  const ctx = await b.newContext({ viewport: { width, height: 844 }, deviceScaleFactor: 2, isMobile: true });
  ...
  if (MUTATE === "empty-grew")  await page.addStyleTag({ content: '.field--bar[data-state="empty"]{border-radius:16px !important}' });
  ...
  const box = await at(".box"), area = await at("#ownText"), clip = await at(".field__attach"), go = await at(".field__go");
  cells += 4;
  ...
  // B. a five-row block of text is a card, not an oval.
  ...
  // D. THE GUARD. The empty bar is the whole reason the bar density exists.
  await page.fill("#ownText", "");
  await page.waitForTimeout(600);
  const empty = await at(".box"), er = await rad(".box");
  cells += 2;
  if (parseFloat(er) < 40) note(t, `the EMPTY bar lost its pill (${er}) — the one-line hero is collateral`);
...
say(`check_field_phone: ${cells} assertions over 3 phone widths`);
